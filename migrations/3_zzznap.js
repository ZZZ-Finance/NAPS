const ZZZNAP = artifacts.require("NonMintableV3");
const { constructors, multipliers } = require("../helper");

module.exports = async function (deployer, network, accounts) {
  const constructor = constructors.ZZZNAP;
  const constructorArgs = Object.entries(constructor).map(
    ([key, value]) => value
  );
  await deployer.deploy(ZZZNAP, ...constructorArgs);
  let Pool = new web3.eth.Contract(ZZZNAP.abi, ZZZNAP.address);
  ...
  const multis = multipliers.ZZZNAP;
