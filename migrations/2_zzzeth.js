const ZZZETH = artifacts.require("NonMintableV3");
const { constructors, multipliers } = require("../helper");

module.exports = async function (deployer, network, accounts) {
  const constructor = constructors.ZZZETH;
  const constructorArgs = Object.entries(constructor).map(
    ([key, value]) => value
  );
  await deployer.deploy(ZZZETH, ...constructorArgs);
  let Pool = new web3.eth.Contract(ZZZETH.abi, ZZZETH.address);
  ...
  const multis = multipliers.ZZZETH;
